...
import com.swIng.server.webResources.AdminAuthentication;
import com.swIng.server.webResources.FlashMobAdministrator;
import com.swIng.server.webResources.FlashMobAuthorizationManagment;
import com.swIng.server.webResources.FlashMobList;
import com.swIng.server.webResources.FlashMobPictureStorageResource;
import com.swIng.server.webResources.FlashMobResourceForManagment;
import com.swIng.server.webResources.UserAuthentication;
import com.swIng.server.webResources.UserRegistration;
...
public class MyWebApp extends Application {
	...
	public Restlet createInboundRoot() {
		...
		try {
			FileReader fr = new FileReader("admins.json");
			User[] admins = new Gson().fromJson(fr, User[].class);
			fr.close();
			if (admins != null) {
		...
		try {
			...
			MyUser[] users = new Gson().fromJson(fr, MyUser[].class);
			fr.close();
		...
		ChallengeAuthenticator guardGenericUser = new ChallengeAuthenticator(getContext(), ChallengeScheme.HTTP_BASIC,
				"MyRealm") {
		...
		ChallengeAuthenticator guardUserForFlashMob = new ChallengeAuthenticator(getContext(),
		...
		ChallengeAuthenticator guardUserAuthForFlashMob = new ChallengeAuthenticator(getContext(),
				ChallengeScheme.HTTP_BASIC, "MyRealm");
	...
	private ChallengeAuthenticator createAuthenticator() {
		ChallengeAuthenticator guard = new ChallengeAuthenticator(getContext(), ChallengeScheme.HTTP_BASIC, "MyRealm");
